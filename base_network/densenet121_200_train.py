...
from keras.layers import Dense, Dropout, Flatten, Activation, Concatenate, Input, Reshape, GlobalAveragePooling2D, GlobalMaxPooling2D
...
df_train = pd.read_csv('../input/initial_train_200_classes.csv')
df_val = pd.read_csv('../input/initial_val_200_classes.csv')
...
base_model = DenseNet121(input_shape=(input_size, input_size, 3), weights='imagenet', include_top=False)
...
callbacks = [lrate, EarlyStopping(monitor='val_loss',
             ...
             ModelCheckpoint(monitor='val_loss',
                             filepath='weights/densenet121_200_classes.{epoch:02d}-{val_loss:.5f}.hdf5',
                             save_best_only=True,
                             save_weights_only=True),
             TensorBoard(log_dir='logs'),
             CSVLogger('logs/densenet121_epochlog_200_classes{}.csv'.format(start_time), separator=',')]

training_generator = DataGenerator(train_imgs, df_train['label'])
validation_generator = DataGenerator(val_imgs, df_val['label'])
...
model.load_weights('weights/densenet121_200_classes.31-1.67018.hdf5')
...
model.load_weights('weights/densenet121_200_classes.15-0.23897.hdf5')

model = Model(inputs= [model.input], outputs=model.layers[-4].output)
model.save_weights('weights/densenet121_200_classes.15-0.23897_head_weights_only.hdf5')
